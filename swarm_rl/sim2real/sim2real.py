...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, choices=['single', 'attention'], default='attention',
...
def load_sf_model(model_dir: Path, model_type: str):
    ...
    args.quads_encoder_type = 'corl'#'attention' if model_type == 'attention' else 'corl'
...
def generate_c_model_attention(model: nn.Module, output_path: str, output_folder: str, testing=False):
    ...
    for enc_name, data in info['encoders'].items():
        ...
        if not ('self' in enc_name or 'obst' in enc_name):
            continue
